...
void UnaryExpr::genCode()
{
    ...
    {
        // icmp的结果到底是不是都是i1？？
        if(expr->getSymPtr()->getType()==TypeSystem::intType&&expr->IsId())
        {
            SymbolEntry*se=new TemporarySymbolEntry(TypeSystem::boolType,SymbolTable::getLabel());
            expr=new BinaryExpr(se,BinaryInstruction::NOTEQ,expr,expr0);
            expr->genCode();
...
void Id::genCode()
{
    ...
    //     ConstantSymbolEntry*zeroSe=new ConstantSymbolEntry(TypeSystem::intType,0);
    //     ExprNode *expr0=new Constant((SymbolEntry*)zeroSe);
    //     TemporarySymbolEntry*temp=new TemporarySymbolEntry(TypeSystem::intType,SymbolTable::getLabel());
    //     new CmpInstruction(CmpInstruction::NE,new Operand(temp),dst,new Operand(zeroSe),bb);
